 ...
 *
 * A visitor can only add one like per page: after a successful click the
 * slug is recorded in localStorage and the button switches to a disabled,
 * "liked" state. This is a soft guard (clearing localStorage or using a
 * different browser resets it) -- there's no login system on this blog,
 * so it's the simplest reasonable anti-spam measure.
 */
...
const SCRIPT_LINES = [
  ...
  "  try {",
  "    alreadyLiked = window.localStorage.getItem(storageKey) === '1'",
  "  } catch (e) {}",
  "",
  ...
  "  }",
  "",
  "  applyState(undefined, alreadyLiked)",
  "",
  "  fetch('/api/like/' + encodeURIComponent(slug))",
  "    .then(function (res) { return res.json() })",
  "    .then(function (data) {",
  "      if (typeof data.count === 'number') applyState(data.count, alreadyLiked)",
  "    })",
  "    .catch(function () {})",
  "",
  "  function onClick() {",
